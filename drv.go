...
// Package oracledb is a database/sql/driver for Oracle DB.
//
// The connection string for the sql.Open("oracledb", connString) call can be
// the simple
...
func init() {
	defaultDrv = newDrv()
	sql.Register("oracledb", defaultDrv)
}
...
type VersionInfo = internal.VersionInfo

type drv struct {
	clientVersion VersionInfo
	mu            sync.Mutex
	dpiContext    *internal.CdpiContext
	pools         map[string]*connPool
}

type connPool struct {
	dpiPool       *internal.CdpiPool
