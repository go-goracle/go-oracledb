...
func (conn *Conn) NewQueue(name, payloadObjectTypeName string) (*Queue, error) {
	var payloadType *C.dpiObjectType
	Q := Queue{name: name, Conn: conn}
	var err error
	...
	if C.dpiConn_newQueue(conn.dpiConn, value, C.uint(len(name)), payloadType, &Q.dpiQueue) == C.DPI_FAILURE {
		err = errors.Errorf("newQueue %q: %w", name, Q.Conn.Err())
	}
...
func (Q *Queue) Release() error {
	...
	if C.dpiQueue_release(q) == C.DPI_FAILURE {
		return errors.Errorf("release: %w", c.Err())
	}
	return nil
}
...
func (Q *Queue) EnqOptions() (EnqOptions, error) {
	...
	if C.dpiQueue_getEnqOptions(Q.dpiQueue, &opts) == C.DPI_FAILURE {
		return E, errors.Errorf("getEnqOptions: %w", Q.Conn.Err())
	}
	err := E.fromOra(Q.Conn, opts)
	return E, err
}
...
func (Q *Queue) DeqOptions() (DeqOptions, error) {
	...
	if C.dpiQueue_getDeqOptions(Q.dpiQueue, &opts) == C.DPI_FAILURE {
		return D, errors.Errorf("getDeqOptions: %w", Q.Conn.Err())
	}
	err := D.fromOra(Q.Conn, opts)
	return D, err
}
...
func (Q *Queue) Enqueue(messages []Message) error {
	...
	for i, m := range messages {
		if C.dpiConn_newMsgProps(Q.Conn.dpiConn, &props[i]) == C.DPI_FAILURE {
			return errors.Errorf("newMsgProps: %w", Q.Conn.Err())
		}
	...
	if ok == C.DPI_FAILURE {
		return errors.Errorf("enqueue %#v: %w", messages, Q.Conn.Err())
	}
...
func (M *Message) toOra(d errorer, props *C.dpiMsgProps) error {
	var firstErr error
	OK := func(ok C.int, name string) {
		...
		if firstErr == nil {
			firstErr = errors.Errorf("%s: %w", name, d.Err())
		}
	}
	if M.Correlation != "" {
...
func (M *Message) fromOra(c *Conn, props *C.dpiMsgProps, objType *ObjectType) error {
	...
	OK := func(ok C.int, name string) bool {
		...
		if firstErr == nil {
			firstErr = errors.Errorf("%s: %w", name, c.Err())
		}
		return false
	}
	M.NumAttempts = 0
...
func (E *EnqOptions) fromOra(d errorer, opts *C.dpiEnqOptions) error {
	var firstErr error
	OK := func(ok C.int, msg string) bool {
		...
		if firstErr == nil {
			firstErr = errors.Errorf("%s: %w", msg, d.Err())
		}
		return false
	}
...
func (E EnqOptions) toOra(d errorer, opts *C.dpiEnqOptions) error {
	var firstErr error
	OK := func(ok C.int, msg string) bool {
		if ok == C.DPI_SUCCESS {
			return true
...
func (Q *Queue) SetEnqOptions(E EnqOptions) error {
	var opts *C.dpiEnqOptions
	if C.dpiQueue_getEnqOptions(Q.dpiQueue, &opts) == C.DPI_FAILURE {
		return errors.Errorf("getEnqOptions: %w", Q.Conn.Err())
	}
	return E.toOra(Q.Conn, opts)
}
...
func (D *DeqOptions) fromOra(d errorer, opts *C.dpiDeqOptions) error {
	var firstErr error
	OK := func(ok C.int, msg string) bool {
		if ok == C.DPI_SUCCESS {
			return true
...
func (D DeqOptions) toOra(d errorer, opts *C.dpiDeqOptions) error {
	var firstErr error
	OK := func(ok C.int, msg string) bool {
		if ok == C.DPI_SUCCESS {
			return true
...
func (Q *Queue) SetDeqOptions(D DeqOptions) error {
	var opts *C.dpiDeqOptions
	if C.dpiQueue_getDeqOptions(Q.dpiQueue, &opts) == C.DPI_FAILURE {
		return errors.Errorf("getDeqOptions: %w", Q.Conn.Err())
	}
	return D.toOra(Q.Conn, opts)
}
...
func (Q *Queue) SetDeqCorrelation(correlation string) error {
	...
	C.free(unsafe.Pointer(cs))
	if !ok {
		return errors.Errorf("setCorrelation: %w", Q.Conn.Err())
	}
	return nil
}
