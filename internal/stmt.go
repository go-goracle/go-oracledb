...
func ParseOnly() Option { return parseOnly }
...
func MagicTypeConversion() Option { return func(o *stmtOptions) { o.magicTypeConversion = true } }
...
func CallTimeout(d time.Duration) Option { return func(o *stmtOptions) { o.callTimeout = d } }
...
func (st *statement) ExecPre(args []driver.NamedValue) (C.dpiExecMode, error) {
...
func (st *Stmt) ExecDo(mode C.dpiExecMode) error {
	...
	if C.dpiStmt_getInfo(st.dpiStmt, &info) == C.DPI_FAILURE {
		err = errors.Errorf("getInfo: %w", st.Err())
	}
	st.isReturning = info.isReturning != 0
	return err
}

func (st *Stmt) ExecPost() (driver.Result, error) {
...
func (st *Stmt) QueryPre(args []driver.NamedValue) (C.dpiExecMode, error) {
	st.isReturning = false
	st.conn.RLock()
...
func (st *Stmt) QueryDo(mode C.dpiExecMode) (int, error) {
	// execute
	var colCount C.uint32_t
	if C.dpiStmt_execute(st.dpiStmt, mode, &colCount) == C.DPI_FAILURE {
		return st.Err()
...
func (st *Stmt) QueryPost(int colCount) (driver.Rows, error) {
	return st.openRows(colCount)
}
